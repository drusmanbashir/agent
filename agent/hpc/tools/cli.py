...
COMMON_HPC_CONFIG_NAME = "config_hpc.yaml"
...
def _conf_path(name: str) -> Path:
    conf_root = os.environ.get(CONFIG_ENV)
    if not conf_root:
        raise ConfigError(f"${CONFIG_ENV} is not set; cannot locate {name}.")
    return Path(conf_root).expanduser() / name
...
def _load_yaml_mapping(path: Path) -> dict[str, str]:
    if not path.exists():
...
def _map_local_path_to_hpc_path(source_path: str) -> str:
    source = os.path.normpath(str(Path(source_path).expanduser()))
    try:
        local_conf = _load_yaml_mapping(_conf_path(COMMON_CONFIG_NAME))
        remote_conf = _load_yaml_mapping(_conf_path(COMMON_HPC_CONFIG_NAME))
    except ConfigError:
        return posixpath.normpath(source)

    candidates: list[tuple[int, str, str]] = []
    for key in sorted(set(local_conf) & set(remote_conf)):
        local_raw = local_conf.get(key)
        remote_raw = remote_conf.get(key)
        if not local_raw or not remote_raw:
            continue
        if not str(local_raw).startswith("/") or not str(remote_raw).startswith("/"):
            continue
        local_prefix = os.path.normpath(str(local_raw))
        if not _path_has_prefix(source, local_prefix):
            continue
        rel = _rel_from_prefix(source, local_prefix).replace(os.sep, "/")
        remote_prefix = posixpath.normpath(str(remote_raw))
        remote_path = remote_prefix if not rel else posixpath.join(remote_prefix, rel)
        candidates.append((len(local_prefix), remote_path, key))
    ...
    if len(best) > 1:
        details = ", ".join(f"{key}:{path}" for _, path, key in best)
        raise ConfigError(f"Ambiguous local->HPC mapping for '{source}': {details}")
    return best[0][1]


def _resolve_remote_spec(raw_spec: str, config_path: Path | None = None, host_alias: str = "hpc") -> str:
    spec = os.path.expandvars(os.path.expanduser(raw_spec.strip()))
    if not spec:
        raise ConfigError("Remote spec cannot be empty.")

    config = _load_hpc_config(config_path)
    login = config["login"]

    if spec.startswith(f"{login}:"):
        return spec
    if spec.startswith(f"{host_alias}:"):
        return f"{login}:{spec.split(':', 1)[1]}"
    if ":" in spec and not spec.startswith("/"):
        return spec

    suffix = f"@{host_alias}"
    if spec.endswith(suffix):
        path_part = spec[: -len(suffix)]
        if not path_part:
            raise ConfigError(f"Missing path before {suffix}.")
        if not path_part.startswith("/"):
            raise ConfigError(f"Remote path before {suffix} must expand to an absolute path: {raw_spec}")
        return f"{login}:{_map_local_path_to_hpc_path(path_part)}"

    if spec.startswith("/"):
        return f"{login}:{_map_local_path_to_hpc_path(spec)}"

    raise ConfigError(
        f"Unsupported remote spec '{raw_spec}'. Use /abs/path, /abs/path@{host_alias}, or user@host:/abs/path."
    )
...
def _run_resolve_remote_spec(config_path: Path | None, spec: str, host_alias: str) -> int:
    try:
        resolved = _resolve_remote_spec(spec, config_path=config_path, host_alias=host_alias)
    except ConfigError as exc:
        return _print_config_error(exc)
    print(resolved)
    return 0
...
def main(args: argparse.Namespace) -> int:
    ...
    if args.cmd == "resolve_remote_spec":
        return _run_resolve_remote_spec(
            config_path=args.config,
            spec=args.spec,
...
if __name__ == "__main__":
    ...
    resolve = sub.add_parser(
        "resolve_remote_spec",
        help="Resolve /abs/path or $VARS@hpc shorthand into rsync-ready user@host:/abs/path.",
    )
    resolve.add_argument("spec", help="Remote path spec such as /path, $COLD_STORAGE@hpc, or user@host:/path.")
    resolve.add_argument("--config", type=Path, default=None, help="Path to hpc.yaml (default: $FRAN_CONF/hpc.yaml).")
    resolve.add_argument(
        "--host-alias",
        default="hpc",
        help="Alias used by @alias and alias:/path shorthand (default: hpc).",
    )
